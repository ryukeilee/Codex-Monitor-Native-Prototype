...
- 菜单栏标题只显示周额度剩余百分比。
- Popover 展示更完整的额度、恢复时间、建议和数据可信度信息。
...
- 成功、刷新中、已过期、网络异常、需要登录、数据异常时：
  菜单栏优先显示上次成功真实快照里的周额度百分比，例如 `71%`。
- 尚无真实快照、空闲或演示模式时：
  菜单栏显示 `--%`，避免把演示数据误当成真实额度。

这意味着菜单栏是“最小状态面板”，只负责持续给出一个稳定的周额度数字，不展开更多解释。
...
- Quota Summary 主体：
  显示决策等级、5 小时额度、周额度、预计恢复时间、剩余恢复时长和建议文案。
...
- 菜单栏是否继续保留周额度或 `--%`
- Popover 顶部是否显示更新时间、尝试时间、数据来源和状态
- 失败时是否保留上次成功快照
...
- 菜单栏标题只显示周额度，不显示 5 小时额度。
- Popover 会显示更新时间、最近尝试时间、数据来源与刷新状态。
