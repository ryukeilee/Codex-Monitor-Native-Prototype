...
- Quota Summary 主体：
  按 `5 小时 → 周 → 月 → 未知` 的稳定顺序展示当前实际存在的窗口；每项的数字、进度、字段状态、恢复时间和缓存标记来自同一投影。未知窗口保留“未知额度”语义并以稳定序号区分，不猜成周或月。

兼容与容量规则：

- 服务端动态窗口缺少 5 小时或周窗口时，仅在对应 legacy 字段本身可信时补入兼容项；无效字段和演示值不会显示成真实百分比。
...
- Popover、tooltip 与 Widget 共用动态窗口投影，覆盖 only-weekly、only-monthly、only-unknown、已知三窗口、多个额外窗口、缓存和无效值路径。
