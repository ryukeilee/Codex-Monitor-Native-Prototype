...
最后更新：2026-08-21（Loop 13 额度卡状态迁移动画；测试数量 577→583）
...
- **Popover 状态迁移动画**：额度区状态切换（刷新结果、缓存翻转、恢复边界、卡片增删）用 reduce-motion 感知的作用域动画平滑过渡——QuotaSummaryView 键控卡片 id 列表（卡片增删）、QuotaGaugeView 键控 item（卡内数值/进度/徽标）、header 指示器键控 effectiveStatus（固定尺寸交叉淡入淡出）；动画不改变静态布局与辅助功能契约。注意 popover 的 \`historyCaption\` 是死路径（显示路径自 18d7674 起只保留 .isCurrent==.live 窗口，caption 恒为 nil），勿再为其做布局设计。
