...
最后更新：2026-09-15（Loop 16 Popover 呈现失败回滚；测试数量 583）
...
- **周额度趋势与异常提醒**：主 App 通过独立的 \`UsageTrendStore\` 按账号/会话边界保存可信真实周额度样本，不进入共享 Widget payload。\`UsageTrendAnalyzer\` 只分析同一连续序列；额度重置与异常跳变都会开启新基线，至少 3 个样本且跨度 10 分钟后才计算速度与耗尽预测，且采用近期加权与稳健抑制：长空闲（≥90 分钟且消耗 ≤1%）后仅用空闲后窗口，近期（≥30 分钟或 4 点）显著更快时融合近期速率，对总消耗 <2%、速率 <0.7%/h、高残差或 >14 天远期的预测抑制为 \`insufficientData\`/\`stable\` 不误导；对不规则间隔按时间加权。非重置上涨、30 分钟内下降超过 20 个百分点或单次下降超过 50 个百分点会产生一次 \`QuotaAnomaly\`，由主 App 请求系统通知权限并提醒；重置、重复、乱序、mock 或账号边界不匹配的数据不提醒。
- **安装/单实例**：\`AppInstallationAuthority\` 校验安装身份；\`SingleInstanceCoordinator\` 仲裁单实例所有权（redirect 首选安装、拒绝无效安装、版本与签名锚点比较）。
- **Popover 状态迁移与失败回滚**：额度区状态切换（刷新结果、缓存翻转、恢复边界、卡片增删）用 reduce-motion 感知的作用域动画平滑过渡——QuotaSummaryView 键控卡片 id 列表（卡片增删）、QuotaGaugeView 键控 item（卡内数值/进度/徽标）、header 指示器键控 effectiveStatus（固定尺寸交叉淡入淡出）；动画不改变静态布局与辅助功能契约。`PopoverController` 在 AppKit 拒绝 `NSPopover.show` 时必须回滚 presentation state、panel active、layout task 与 event monitors，不得留下半打开资源。注意 popover 的 \`historyCaption\` 是死路径（显示路径自 18d7674 起只保留 .isCurrent==.live 窗口，caption 恒为 nil），勿再为其做布局设计。
...
- \`AGENTS.md\` 中的测试数量以 \`swift test\` 实际结果为准。